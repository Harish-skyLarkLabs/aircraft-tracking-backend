...
# Legacy IOUTracker for backwards compatibility
class IOUTracker:
    """
    Simple IOU-based tracker for aircraft detection
    Kept for backwards compatibility - prefer AircraftTracker for new code
    """
    ...
    def __init__(
        ...
        self.max_age = max_age
        self.min_hits = min_hits
        self.tracks: Dict[int, TrackedAircraft] = {}
        self.next_track_id = 1
        self.frame_count = 0
    ...
    def update(
        ...
        if len(detections) > 0 and len(self.tracks) > 0:
            matched, unmatched_dets, _ = self._associate_detections_to_tracks(
                detections, list(self.tracks.values())
            )
            
            # Update matched tracks
            for det_idx, track_id in matched:
                self.tracks[track_id].update_detection(detection_dicts[det_idx], self.frame_count)
            
            # Create new tracks for unmatched detections
            for det_idx in unmatched_dets:
                self._create_track(detection_dicts[det_idx])
        ...
        results = []
        for track_id, track in self.tracks.items():
            if track.detection_count >= self.min_hits or self.frame_count <= self.min_hits:
                results.append((track_id, track.last_position))
    ...
    def _associate_detections_to_tracks(
        ...
        for d, det in enumerate(detections):
            for t, track in enumerate(tracks):
                iou_matrix[d, t] = calculate_iou(det, track.last_position)
    ...
    def _create_track(self, detection: dict):
        """Create a new track"""
        track = TrackedAircraft(self.next_track_id, detection, self.frame_count)
